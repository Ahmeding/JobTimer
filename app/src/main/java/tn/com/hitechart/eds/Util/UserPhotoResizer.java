...
/**
 * Created by BARA' on 04/03/2016.
 */
public class UserPhotoResizer {
     ...
     * width and height
     */
    public static Bitmap decodeSampleBitmapfromFil(String filename, int reqWidth, int reqHeight) {
        // First we must decod with inJustDecodeBounds=true to check dimension
        final BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(filename, options);
        //Calculate inSampleSize
        options.inSampleSize = calculateInSampleSize(options, reqWidth, reqHeight);
        //decode bitmap with insipmlesize
